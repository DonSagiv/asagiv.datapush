...
namespace asagiv.pushrocket.interop.Platforms.Android
{
    public static class FilesHelper
    {
        ...
        public static string GetActualPathForFile(Uri uri, Context context)
        {
                ...
                else if (IsDownloadsDocument(uri))
                {
                    try
                    {
                        ...
                        if (!TextUtils.IsEmpty(id))
                        {
                            ...
                            {
                                var contentUri = ContentUris.WithAppendedId(Uri.Parse(contentUriPrefix), long.Parse(id));
                            ...
                            return string.IsNullOrEmpty(path)
                                ? Environment.ExternalStorageDirectory.ToString() + "/Download/" + GetFileName(context, uri)
                                : path;
                        }
                    }
                    catch
                    {
                        return Environment.ExternalStorageDirectory.ToString() + "/Download/" + GetFileName(context, uri);
                    }
                }
                // MediaProvider
